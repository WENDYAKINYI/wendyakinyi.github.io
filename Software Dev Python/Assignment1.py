...
def retrieve_student_number(student_name, student_id_dict):
    """
    This function retrieves a student's id number from the dictionary of student ids and also handles misspelled names by searching for a close match.
    ...
    # Handling misspelled names by finding a close match
    for name in student_id_dict.keys():
        if name.lower().startswith(student_name.lower()[:3]):
            return student_id_dict[name]

    # If no close match is found, return student name not found
...
# Test, sing until the user stops
if __name__ == '__main__':
    un_kilometre_a_pied()
...
class Tarp(Shelter):
    ...
    def __repr__(self):
        ...
        A detailed string representation of the tarp object.
        """
        return (f"Tarp({self.num_occupants}, {self.material}, {self.setup_time}, {self.sqft}, {self.weight}, {self.seasons})")
